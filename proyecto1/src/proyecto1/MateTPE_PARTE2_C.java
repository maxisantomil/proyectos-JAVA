...
public class MateTPE_PARTE2_C {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("ingrese un numero G");
			G = new Double(entrada2.readLine());
			int i = mediarTanque(h, deltaT, t, entrada, A, C,G);
			mostrar("Tanque a la mitad en (segundos) ", i);
			int ti = llenarTanque(h, deltaT, t, entrada, A, C, G);
			mostrar("Tanque lleno en (segundos) ", ti);
			int tv = vaciarTanque(h, deltaT, A, C, ti,G);
	...
	public static double salida_3(double G, double A, double HT) {
		double nueva_salida;
		nueva_salida = ((G *A) *(HT));
		return nueva_salida;
	}

	public static int llenarTanque(double[] h, int deltaT, int t, double entrada, double A, double C, double G) {
		int i=0;
		while (t < MAX) {
			h[t] = h[t - 1] + (entrada - (salida_3(G,A,h[t]))) * (deltaT / A);
			int ht = (int) h[t];
			...
			if (ht == C) {
				...
				t = MAX;
					...
					if  (ht> C) 
						System.out.println("el tanque se lleno y  rebalso ");
						t= MAX;
				}
			t++;
		}
	...
	public static int mediarTanque(double[] h, int deltaT, int t, double entrada, double A, double C,double G) {
		int i = 0;
		while (t < MAX) {
			h[t] = h[t - 1] + (entrada - (salida_3(G,A,h[t]))) * (deltaT / A);
			int ht = (int) h[t];
	...
	public static int vaciarTanque(double[] h, int deltaT, double A, double C, int ti, double G) {
		int tv = 0;
		double entrada = 0;
		while (ti < MAX) {
			h[ti] = h[ti - 1] + (entrada - (salida_3(G,A,h[ti]))) * (deltaT / A);
			int ht = (int) h[ti];
			if (ht == 0) {
				tv = ti;
				ti = MAX;
